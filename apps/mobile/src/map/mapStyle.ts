 ...
 *
 * Ported verbatim from apps/web/src/web/mapStyle.ts — the only change is the
 * import source for `StyleSpecification` (re-exported by
 * @maplibre/maplibre-react-native from @maplibre/maplibre-gl-style-spec,
 * the same spec type the web app's `maplibre-gl` uses). The recolour/filter
 * logic below is platform-independent: it fetches OpenFreeMap's live style
 * JSON and patches it, which works identically whether the result is handed
 * to MapLibre GL JS or MapLibre Native.
 *
